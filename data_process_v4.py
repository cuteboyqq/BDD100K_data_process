...
def Get_Fit_label(mask_dri,split_lane_map,mask_line,drive_mapping,file):
    ...
    w=640
    h=360
    '''resize, maybe yolo-adas do not need to resize'''
    ...
    '''initial the output mask'''
    mask_out = np.zeros(mask_dri.shape)
    '''relabel to 0 255'''
    #mask_dri = map_labels(mask_dri, drive_mapping, bg_lb)
    mask_dri[mask_dri == 1] = 0
    mask_dri[mask_dri == 2] = 255
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (9, 9))
    mask_dri = cv2.dilate(mask_dri, kernel=kernel)
    #mask_dri = cv2.bitwise_not(mask_dri)
    line_tmp = copy.deepcopy(mask_line)
    line_tmp[mask_line == 170] = 0
    line_tmp[mask_line == 225] = 0
    mask_out = cv2.bitwise_and(mask_dri, line_tmp)
    
    

    mask_out[mask_out == 0] = split_lane_map[mask_out == 0]
    mask_out[mask_out > 0] = split_lane_map[mask_out > 0]
    #            print(mask_out[i][j])
...
def parsing_imgs_and_labels(data_dir):
    ...
    for i in range(len(train_lane_mask_list)):
        ...
        '''New code'''
        split_lane_map_ver2 = map_labels(split_lane_map,new_line_mapping,lane_bg)
      
        save_line = "./line"
        if not os.path.exists(save_line):
            os.makedirs(save_line)
        if not os.path.exists(os.path.join(save_line,file)):
            cv2.imwrite(save_line+'/'+file,split_lane_map_ver2)
        else:
            print("file {} exists! pass it".format(split_lane_map))
            
        '''Step 3: Get fit line label'''
        #split_lane_map ===> need to remapping to original map labels
        #split_lane_map_ver2 ===> for do AND function use :0 , 255
